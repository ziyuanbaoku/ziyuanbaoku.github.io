			...
			$(this).click(function(){
				...
				$.each(listNews,function(index,value){
					...
					if(index==0){
                       ...
                       '</em><span>'+(index+1) + '</span><a target="blank" href="'+ value.url +'">' + value.title + '</a></li><div class="alert alert-warning" style="margin-bottom:0"><button type="button" class="close" data-dismiss="alert"><span>&times;</span></button><strong>百度网盘链接：</strong> <p style="word-wrap:break-word">'+ value.url +'</p><strong>验证码：</strong>'+ value.yzm +'</div>'
					} else {
					   //生成列表的HTML标签					
